...
def language_tags(create_data= False):
    ...
    if create_data is True:
        post_tag = di.get_post_common_tag(file_read = fp.posts_file_xml)
        pickle.dump(post_tag, open("data/post_common_tag.pkl", "wb"))
    else:
        post_tag = pickle.load(open("data/post_common_tag.pkl", "rb"))
    with open('data/language_common_topics.csv','w') as out_file:
        writer= csv.writer(out_file)
        writer.writerow(["language_pair", "topic", "count"])
        for lang_pair in post_tag:
            print(lang_pair)
            tag_counter = post_tag[lang_pair]
            top_tags = tag_counter.most_common(30)
            for tag in top_tags:
                writer.writerow([lang_pair, tag[0], tag[1]])
